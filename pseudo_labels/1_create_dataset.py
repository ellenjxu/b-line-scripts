"""
Generate data and folder structure:
...
with the corresponding .tiffs based on the split_dictionary.csv.
...
if not os.path.isdir(os.path.join(output_folder, 'frames_16', '0')):
    os.mkdir(os.path.join(output_folder, 'frames_16', '0'))
else:
...
for split in ["0", "test"]:
    ...
    if split == "0":
        cases = split_df.loc[split_df['split'] == "train"]
    elif split == "test":
        cases = split_df.loc[split_df['split'] == "test"]
