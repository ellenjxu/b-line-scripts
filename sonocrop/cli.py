...
def edges(input_file, thresh=0.1):
  ...
  maxW = np.max(u_avg, axis=0) # get the maximum values per column (width)
  left,right = vid.findEdges(maxW, thresh=thresh)
  maxH = np.max(u_avg, axis=1) # get the maximum values per row (height)
  top,bottom = vid.findEdges(maxH, thresh=thresh)

  return (f'{left},{right},{top},{bottom}')


def mask(input_file, output_file, thresh=0.05, save_mask=False):
...
#   rich.print(f'  Thresh: {thresh}')
  ...
  u_avg = u/f
  mask = u_avg > thresh
  # print(u_avg[280][1162]) # [y coord of pixel][x coord of pixel]
...
def get_mask(input_file, thresh=0.05):
